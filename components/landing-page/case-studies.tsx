        ...
        <div className="grid md:grid-cols-3 gap-6">
          {cases.map((caseStudy, idx) => {
            ...
            return (
              <Card
                key={idx}
                className={`overflow-hidden bg-white/70 hover:bg-white hover:shadow-2xl transition-all duration-300 group cursor-pointer transform hover:-translate-y-2 ${
              ...
              >
                ...
                <div className="relative h-48 overflow-hidden bg-muted">
                  ...
                  <div className="absolute top-4 right-4 bg-black text-accent-foreground px-3 py-1 rounded-full text-xs font-semibold flex items-center gap-1">
                    <Icon size={14} />
                    {caseStudy.category}
                  </div>
                </div>
                ...
                <div className="p-6">
                  <div className="flex items-start justify-between mb-3">
                    <h3 className="text-lg font-bold text-primary flex-1 group-hover:text-amber-500 transition-colors">{caseStudy.title}</h3>
                  </div>
                  
                  <p className="text-sm text-foreground/60 mb-3 font-semibold">Client: {caseStudy.client}</p>
                  <p className="text-xs text-foreground/50 mb-4">{caseStudy.details}</p>
